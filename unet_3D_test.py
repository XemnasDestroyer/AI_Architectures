...
from config import (BACKGROUND_AS_CLASS, NUM_CLASSES, BCE_WEIGHTS, transforms)
...
def train(device, data_path, model_path, NUM_EPOCHS):
    ...
    model = UNet3D_v2(in_channels=1, num_classes=1).to(device)
    ...
    # La función de pérdida con pesos para manejar el desbalance de clases
    weight_tumor = torch.tensor([9.0]).to(device) 
    criterion = torch.nn.BCEWithLogitsLoss(pos_weight=weight_tumor)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        model.train()
        epoch_loss = 0
        dice_metric.reset() # Resetear al inicio de cada época
        ...
        for batch_data in train_loader:
            ...
            # --- CÁLCULO DEL % DE ACIERTO (DICE) ---
            # 1. Aplicamos Sigmoid y umbral de 0.5 para tener valores binarios
            preds_binarized = [post_pred(i) for i in torch.sigmoid(outputs)]
            # 2. Calculamos el solapamiento
            dice_metric(y_pred=preds_binarized, y=labels)

            # --- VISUALIZACIÓN EN TIEMPO REAL ---
            # if (epoch + 1) % 20 == 0 and idx == 0:
            #     visualizar_progreso(inputs, labels, outputs, epoch + 1)

        # 1. Calculamos el promedio del loss en esta época
        ...
        # Le decimos al scheduler: "Mira este loss, ¿crees que debemos bajar el LR?"
        scheduler.step(avg_loss)
        current_lr = optimizer.param_groups[0]['lr']

        print(f"EPOCH {epoch+1}/{NUM_EPOCHS} | Loss: {avg_loss:.4f} | Dice: {avg_dice:.2f}% | LR: {current_lr:.2e}")

        # Guardar cada 50 épocas o al final
        # if (epoch + 1) % 50 == 0:
        #     save_checkpoint(model, optimizer, scheduler, epoch + 1, checkpoint_path)

    # 7. Guardar los "conocimientos" de la red
...
if __name__ == "__main__":
    ...
    if args.mode == "predict":
        ...
        raise SystemExit("Predicción realizada.")
        ...
        data_path = "./assets/data/3d/brain/00000057_"
        model_path = "./unet3d_brain_model_13032026.pth"
